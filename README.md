...
一个轻量级电商示例项目：商品展示、购物车、订单、地址管理、优惠券、物流追踪、商品推荐。

## 功能一览

**核心交易**：商品搜索（关键字 + 分类 + 价格区间 + 销量排序）、购物车、地址管理、订单创建 / 支付 / 取消 / 确认收货、待支付自动过期取消。

**账户 / 评价**：注册登录 JWT、用户头像上传、商品评价（已完成订单可评）+ 我的评价。

**优惠营销**：优惠券码（fixed / percentage 两种规则），结算时 preview → 下单时服务端再验签 → 取消订单自动退券。

**支付**：沙箱网关（创建意图 + HMAC 回调），UI 上可以选支付宝 / 微信沙箱。

**物流追踪**：发货自动插入第一条轨迹，管理员后台可以随时补录节点，用户订单页展开看时间线。

**管理后台**：`/admin` 角色守卫，Dashboard 六格、订单管理（发货）、商品 CRUD、优惠券管理、物流节点编辑。

**推荐**：商品详情页"猜你喜欢"，基于订单 item 共现度（item-based CF），冷启动回落同分类热卖。
